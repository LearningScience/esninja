...
esninja makes using esbuild faster, simpler and cleaner by inferring entry points from `<script>` and `<link>` tags, using sensible asset loader defaults, adding support for s/css @import using node_module names and maintaining separation between source and output files.
